  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md max-h-[88vh] overflow-y-auto bg-white rounded-3xl shadow-2xl border border-[#C9A227]/25">
        {/* Bandeau */}
        <div className="sticky top-0 z-10 flex items-center justify-between px-5 py-3.5 bg-white/95 backdrop-blur border-b border-[#8A857C]/15">
          <h2 className="text-sm font-bold text-[#000000]">Mon profil</h2>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-stone-100 text-[#8A857C]">
              <X className="w-4 h-4" />
            </button>
          </div>
        </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex flex-col items-center gap-2">
              <div className="relative group">
                {avatarUrl ? (
                  ...
                  <div className="w-24 h-24 rounded-full bg-[#000000] text-white flex items-center justify-center text-2xl font-bold shadow-md">
                    {initials || <UserIcon className="w-8 h-8" />}
                  </div>
                )}
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="absolute -bottom-1 -right-1 w-9 h-9 rounded-full bg-[#C9A227] text-[#000000] flex items-center justify-center shadow-md hover:bg-[#FF7A1A] transition-colors"
              ...
              <div className="flex items-center gap-2 text-[10px] text-[#8A857C]">
                <span>{roleLabel}</span>
                {avatarUrl && (
            ...
            <div className="space-y-3">
              <label className="block">
                <span className="text-[11px] font-bold text-[#8A857C] uppercase tracking-wide">Nom affiché</span>
                <input
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                  maxLength={60}
                  className="mt-1 w-full px-3.5 py-2.5 bg-[#F0E9DE] border border-[#8A857C]/20 rounded-xl text-base md:text-sm text-[#000000] outline-none focus:ring-2 focus:ring-[#C9A227]/40"
                  placeholder="Votre nom"
                />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-[11px] font-bold text-[#8A857C] uppercase tracking-wide flex items-center gap-1"><Phone className="w-3 h-3" />Téléphone</span>
                  <input
                    value={phone}
                ...
                <label className="block">
                  <span className="text-[11px] font-bold text-[#8A857C] uppercase tracking-wide flex items-center gap-1"><MapPin className="w-3 h-3" />Ville</span>
                  <input
                    value={city}
              ...
              <label className="block">
                <span className="text-[11px] font-bold text-[#8A857C] uppercase tracking-wide flex items-center gap-1"><Globe className="w-3 h-3" />Pays</span>
                <input
                  value={country}
              ...
              <label className="block">
                <span className="text-[11px] font-bold text-[#8A857C] uppercase tracking-wide flex items-center gap-1"><FileText className="w-3 h-3" />Bio</span>
                <textarea
                  value={bio}
                  onChange={(e) => setBio(e.target.value)}
                  maxLength={280}
                  rows={3}
                  className="mt-1 w-full px-3.5 py-2.5 bg-[#F0E9DE] border border-[#8A857C]/20 rounded-xl text-base md:text-sm text-[#000000] outline-none focus:ring-2 focus:ring-[#C9A227]/40 resize-none"
                  placeholder="Quelques mots sur vous (visibles par la communauté)…"
                />
                <span className="text-[10px] text-[#8A857C]/70">{bio.length}/280</span>
              </label>
            </div>
            ...
            <div className="flex items-center gap-2 pt-1">
              <button
                ...
                className={cn(
                  ...
                  saving
                    ? "bg-[#C9A227]/50 text-[#000000]/60 cursor-not-allowed"
                    : "bg-[#C9A227] text-[#000000] hover:bg-[#FF7A1A] shadow-md",
                )}
              >
              ...
              <button
                onClick={onClose}
                className="px-4 py-2.5 rounded-xl text-sm font-semibold text-[#8A857C] hover:bg-stone-100 transition-colors"
              >
                Fermer
              </button>
            </div>
