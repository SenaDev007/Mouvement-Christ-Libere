...
/**
 * ============================================================
 * BIBLE WORKSPACE — Composant réutilisable de lecture biblique
 * ============================================================
 *
 * ⭐ Extrait de la page /bible pour être embarqué AILLEURS :
 *    - mode "page"     → route /bible (pleine page, header sticky)
 *    - mode "embedded"  → DANS Yeshua Connect / la communauté :
 *                         plein écran flex sans scroll de page,
 *                         bouton fermer (onClose) et partage de
 *                         verset vers la conversation (onShareVerse).
 *
 * La navigation reste interne : livres, chapitres, versions,
 * recherche, Strong, hébreu, Peshitta — tout s'ouvre ICI.
 */
...
export interface BibleVerseShare {
  reference: string;
  text: string;
}

export interface BibleWorkspaceProps {
  /** "page" = route /bible · "embedded" = intégré dans Yeshua Connect */
  variant?: "page" | "embedded";
  /** En mode embedded : ferme le panneau Bible */
  onClose?: () => void;
  /** Partager un verset (référence + texte) dans la conversation active */
  onShareVerse?: (verse: BibleVerseShare) => void;
}
      ...
      <main
        className={
          isEmbedded
            ...
            : "mx-auto max-w-[1600px] px-3 md:px-6 py-6"
        }
      >
        <div className={isEmbedded ? "mx-auto max-w-[1600px] px-3 md:px-4 py-4" : ""}>
        <AnimatePresence mode="wait">
          ...
          >
            {onglet === "lecture"     && <OngletLecture embedded={isEmbedded} onShareVerse={onShareVerse} />}
            {onglet === "recherche"   && <OngletRecherche embedded={isEmbedded} />}
            {onglet === "strong"      && <OngletStrong embedded={isEmbedded} />}
            {onglet === "hebreu"      && <OngletHebreu embedded={isEmbedded} />}
            {onglet === "peshitta"    && <OngletPeshitta embedded={isEmbedded} />}
            {onglet === "concordance" && <OngletConcordance embedded={isEmbedded} />}
            {onglet === "comparatif"  && <OngletComparatif embedded={isEmbedded} />}
          </motion.div>
        </AnimatePresence>
        </div>
      </main>
...
function BibleHeader({ onglet, setOnglet, embedded = false, onClose }: BibleHeaderProps) {
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-[#C9A227]/20">
        <div className={cn("mx-auto max-w-[1600px] flex items-center gap-3 md:gap-6", embedded ? "px-3 md:px-4 py-2" : "px-3 md:px-6 py-3")}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className={cn(embedded ? "block" : "hidden md:block")}>
              ...
              <p className="text-[9px] uppercase tracking-[0.14em] text-[#C9A227]/80 font-semibold mt-0.5 hidden sm:block">
                6 versions · Strong · Hébreu
              </p>
            </div>
          </div>
          ...
          {/* Actions : marque-pages + fermer (embedded) / paramètres (page) */}
      ...
      <div className={cn("mx-auto max-w-[1600px]", embedded ? "px-3 md:px-4" : "px-3 md:px-6")}>
          ...
          {ONGLETS.map((o) => {
            ...
            return (
              ...
              >
                ...
                <span className={embedded ? "hidden sm:inline" : ""}>{o.label}</span>
              </button>
            );
          })}
        </div>
...
// ============================================================
interface OngletLectureProps {
  embedded?: boolean;
  /** Partager un verset dans la conversation active (Yeshua Connect) */
  onShareVerse?: (verse: BibleVerseShare) => void;
}

function OngletLecture({ embedded = false, onShareVerse }: OngletLectureProps) {
  ...
  return (
    <div className={cn(
      "gap-4",
      embedded
        ? "grid lg:grid-cols-[220px_1fr] xl:grid-cols-[220px_1fr_280px]"
        : "grid lg:grid-cols-[260px_1fr_300px]"
    )}>
      ...
      <aside className={cn(
        ...
        <div className={cn(
          ...
          <div className={cn("overflow-y-auto scrollbar-thin", embedded ? "max-h-[55vh]" : "max-h-[calc(100vh-200px)]")}>
            <ListeLivres
              livreActuel={livre}
      ...
      <aside className="hidden lg:block">
        ...
        >
          ...
          <div className={cn("p-4 overflow-y-auto scrollbar-thin", embedded ? "max-h-[55vh]" : "max-h-[calc(100vh-200px)]")}>
            {versetSelectionne ? (
              <VersetEtude
                livre={data?.livre || ""}
                livreId={livre}
...
// RECHERCHE
// ============================================================
function OngletRecherche({ embedded = false }: { embedded?: boolean }) {
  const [version, setVersion] = useState("fr-apee");
...
// STRONG
// ============================================================
function OngletStrong({ embedded = false }: { embedded?: boolean }) {
...
// HÉBREU
// ============================================================
function OngletHebreu({ embedded = false }: { embedded?: boolean }) {
...
// PESHITTA
// ============================================================
function OngletPeshitta({ embedded = false }: { embedded?: boolean }) {
...
// CONCORDANCE
// ============================================================
function OngletConcordance({ embedded = false }: { embedded?: boolean }) {
  const [numero, setNumero] = useState("");
...
// ============================================================
function OngletComparatif({ embedded = false }: { embedded?: boolean }) {
  const [livre, setLivre] = useState("gn");
  const [chapitre, setChapitre] = useState(1);
  const [verset, setVerset] = useState(1);
