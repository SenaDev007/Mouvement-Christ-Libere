...
export function IntercessionView({ hero }: { hero: HeroConfig }) {
  ...
  return (
    <div>
      ...
      <section className="bg-[#F0E9DE] py-14 md:py-16 border-b border-[#8A857C]/15">
        <div className="container mx-auto max-w-3xl px-4">
          <div className="card-gold-top p-6 md:p-8 flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
            <span className="w-14 h-14 rounded-full bg-[#000000]/5 border border-[#C9A227]/30 flex items-center justify-center flex-shrink-0">
              <Lock className="w-6 h-6 text-[#C9A227]" />
            </span>
            <div>
              <h2 className="font-serif text-lg font-semibold text-[#000000] mb-1.5 flex items-center gap-2 justify-center sm:justify-start">
      ...
      <section id="demander" className="bg-[#F0E9DE] py-20 md:py-24">
        <div className="container mx-auto max-w-2xl px-4">
          {submitted ? (
            ...
            >
              ...
              <h3 className="font-serif text-2xl font-semibold text-[#000000] mb-3">Demande transmise</h3>
              <p className="text-sm text-[#8A857C] mb-6 leading-relaxed">
            ...
            <>
              <div className="text-center mb-8">
                <h2 className="font-serif text-3xl font-semibold text-[#000000] mb-3">Déposer une demande</h2>
              ...
              <form onSubmit={handleSubmit} className="card-gold-top p-6 md:p-8 space-y-5">
                <div>
                  <label htmlFor="intercession-auteur" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Nom complet *</label>
                  <input id="intercession-auteur" type="text" value={form.auteur} onChange={(e) => setForm({ ...form, auteur: e.target.value })} required placeholder="Votre nom et prénom" autoComplete="name" maxLength={100} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                </div>

                {/* ⭐ V3.32 — Localisation : savoir d'où vient la demande */}
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    <label htmlFor="intercession-pays" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Pays *</label>
                    <input id="intercession-pays" type="text" value={form.pays} onChange={(e) => setForm({ ...form, pays: e.target.value })} required placeholder="Ex. : Bénin" autoComplete="country-name" maxLength={100} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                  </div>
                  <div>
                    <label htmlFor="intercession-ville" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Ville</label>
                    <input id="intercession-ville" type="text" value={form.ville} onChange={(e) => setForm({ ...form, ville: e.target.value })} placeholder="Ex. : Cotonou" autoComplete="address-level2" maxLength={100} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                  </div>
                </div>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div>
                    <label htmlFor="intercession-telephone" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Numéro de téléphone</label>
                    <input id="intercession-telephone" type="tel" inputMode="tel" value={form.telephone} onChange={(e) => setForm({ ...form, telephone: e.target.value })} placeholder="Ex. : +229 01 02 03 04 05" autoComplete="tel" maxLength={30} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                  </div>
                  <div>
                    <label htmlFor="intercession-email" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Email</label>
                    <input id="intercession-email" type="email" inputMode="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="votre.email@exemple.com" autoComplete="email" maxLength={150} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                  </div>
                </div>

                <div>
                  <label htmlFor="intercession-categorie" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Catégorie</label>
                  <select id="intercession-categorie" value={form.categorie} onChange={(e) => setForm({ ...form, categorie: e.target.value })} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20">
                ...
                <div>
                  <label htmlFor="intercession-sujet" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">Sujet *</label>
                  <input id="intercession-sujet" type="text" value={form.sujet} onChange={(e) => setForm({ ...form, sujet: e.target.value })} required placeholder="Résumé court de votre demande" maxLength={200} className="w-full min-h-[44px] px-4 py-3 rounded-full border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20" />
                </div>

                <div>
                  <label htmlFor="intercession-description" className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-2 block">
                    Description {audioFile ? "(facultative — votre note vocale sera transmise)" : "*"}
                  </label>
                  <textarea id="intercession-description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required={!audioFile} rows={5} placeholder="Décrivez votre demande en détail, ou enregistrez plutôt une note vocale ci-dessous…" maxLength={2000} className="w-full min-h-[120px] px-4 py-3 rounded-3xl border border-[#8A857C]/30 bg-[#F0E9DE] text-[#000000] placeholder:text-[#8A857C]/60 focus:outline-none focus:border-[#C9A227] focus:ring-2 focus:ring-[#C9A227]/20 resize-y font-serif" />
                  <p className="text-[11px] text-[#8A857C] mt-1.5">
                    {audioFile
                ...
                <label className="flex items-center gap-2.5 cursor-pointer min-h-[44px]">
                  <input type="checkbox" checked={form.isUrgent} onChange={(e) => setForm({ ...form, isUrgent: e.target.checked })} className="w-4 h-4 rounded border-[#8A857C]/30 text-state-danger focus:ring-state-danger" />
                  <span className="text-sm text-[#000000] inline-flex items-center gap-1">
                    <Flame className="w-3.5 h-3.5 text-state-danger" />
                    Marquer comme urgent
                  </span>
                </label>

                <button type="submit" disabled={submitting} className="w-full min-h-[52px] px-6 py-4 rounded-full bg-[#C9A227] text-[#000000] font-semibold text-sm hover:bg-[#FF7A1A] transition-colors inline-flex items-center justify-center gap-2 disabled:opacity-50">
                ...
                <p className="text-[11px] text-[#8A857C] flex items-center justify-center gap-1.5 text-center">
