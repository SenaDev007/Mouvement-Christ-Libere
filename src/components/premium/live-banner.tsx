    ...
    >
      ...
      <div className="container mx-auto max-w-7xl px-4 py-3 flex flex-col sm:flex-row items-center justify-between gap-3 relative z-10">
        <div className="flex items-center gap-3">
          <span className="relative inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] uppercase tracking-[0.2em] font-bold bg-state-danger text-ivory">
        ...
        <Link
          href={href}
          className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-md bg-state-danger text-ivory text-xs font-semibold hover:bg-state-danger/90 transition-colors group"
        >
...
export function NextLiveCard({ title, scheduledAt, servantName, href }: NextLiveCardProps) {
  return (
    <motion.div
      ...
      className="relative bg-imperial text-ivory rounded-card border border-gold/30 overflow-hidden p-7"
    >
      {/* Décor fond */}
      ...
      <div className="relative z-10">
        ...
        <h3 className="font-serif text-xl font-semibold text-ivory mb-3 leading-snug">
          {title}
        </h3>
        ...
        <button className="inline-flex items-center gap-1.5 px-4 py-2 rounded-md bg-gold text-ink text-xs font-semibold hover:bg-gold-light transition-colors">
