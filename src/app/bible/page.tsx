...
export default function BiblePage() {
  ...
  return (
    <div>
      ...
      <section id="bible-app" className="bg-ivory py-12 md:py-16">
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select
          ...
          className="px-3 py-2 rounded-md border border-stone/30 bg-ivory text-ink text-sm focus:outline-none focus:border-gold"
        >
          {VERSIONS.map((v) => (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 rounded hover:bg-gold/10 text-imperial"
            disabled={chapitre <= 1}
          >
            <ChevronLeft className="w-4 h-4" />
          </button>
          <span className="text-sm font-semibold text-ink min-w-[80px] text-center">
            Chapitre {chapitre}
          </span>
          <button
      ...
      {loading ? (
          ...
          <div className="space-y-3">
            {data.versets.map((v) => (
              <div key={v.numero} className="flex gap-3 group">
                <span className="text-xs text-gold-dark font-semibold w-8 text-right pt-0.5 flex-shrink-0">
                  {v.numero}
                </span>
        ...
        <p className="text-stone italic text-center py-12">Chargement...</p>
      )}
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        ...
        <input
          ...
          className="flex-1 px-4 py-2 rounded-md border border-stone/30 bg-ivory text-ink text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
        />
        <button
          onClick={handleSearch}
          className="px-5 py-2 rounded-md bg-gold text-ink text-sm font-semibold hover:bg-gold-light"
        >
          Rechercher
        </button>
      </div>
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto scrollbar-discrete">
            {resultats.map((r, i) => (
              <div key={i} className="card-gold-top p-4">
                ...
                <p className="text-sm text-ink/85 font-serif leading-relaxed">
                  {r.texte}
                </p>
              </div>
            ))}
            {resultats.length === 0 && (
              <p className="text-stone italic text-center py-8">Aucun résultat.</p>
            )}
          </div>
        </div>
      )}
    ...
    <div className="space-y-4">
      ...
      <div className="p-4 bg-imperial/5 border border-gold/20 rounded-md">
        <p className="text-xs text-stone leading-relaxed">
          <strong className="text-ink">Dictionnaire Strong</strong> — 8 674 entrées hébraïques (H1-H8674) et 5 523 entrées grecques (G1-G5523).
        ...
        <div className="card-gold-top p-6">
          <div className="flex items-center gap-2 mb-4">
            <span className={cn(
              ...
              (result.langue as string) === "hebrew" ? "bg-gold/15 text-gold-dark" : "bg-lavender/15 text-lavender"
            )}>
          ...
          {Boolean(result.lemma) && (
            <div className="mb-4">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Lemme</p>
              <p className="font-serif text-2xl text-ink" dir="rtl">{result.lemma as string}</p>
            </div>
          )}

          {Boolean(result.pron) && (
            <div className="mb-3">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Prononciation</p>
              <p className="text-sm text-ink">{result.pron as string}</p>
            </div>
          )}

          {Boolean(result.translit) && (
            <div className="mb-3">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Translittération</p>
              <p className="text-sm text-ink italic">{result.translit as string}</p>
            </div>
          )}

          {Boolean(result.derivation) && (
            <div className="mb-3">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Étymologie</p>
              <p className="text-sm text-ink/80">{result.derivation as string}</p>
            </div>
          )}

          {Boolean(result.strongs_def) && (
            <div className="mb-3">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Définition Strong</p>
              <p className="text-sm text-ink/80 leading-relaxed">{result.strongs_def as string}</p>
            </div>
          )}

          {Boolean(result.kjv_def) && (
            <div className="mb-3">
              <p className="text-xs uppercase tracking-wider text-stone font-semibold mb-1">Traductions KJV</p>
              <p className="text-sm text-ink/70 italic">{result.kjv_def as string}</p>
            </div>
          )}
        </div>
      )}
...
function OngletHebreu() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="p-4 bg-imperial/5 border border-gold/20 rounded-md">
        <p className="text-xs text-stone leading-relaxed">
          <strong className="text-ink">Open Scriptures Hebrew Bible</strong> — Texte massorétique (Westminster Leningrad Codex)
      ...
      {loading ? (
        ...
        <div className="card-gold-top p-6">
          ...
          <div className="space-y-2" dir="rtl">
            {data.mots.map((mot, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded hover:bg-gold/5 transition-colors group">
                <span className="font-serif text-lg text-ink">{mot.mot}</span>
                <div className="flex-1 text-left" dir="ltr">
                  <span className="text-xs font-mono text-gold-dark">{mot.lemme}</span>
                  <span className="text-xs text-stone ml-2">{mot.morphologie}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <p className="text-stone italic text-center py-8">Verset non trouvé.</p>
      )}
    </div>
  );
}
...
function OngletPeshitta() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <input type="text" value={livre} onChange={(e) => setLivre(e.target.value)} className="px-3 py-2 rounded-md border border-stone/30 bg-ivory text-ink text-sm focus:outline-none focus:border-gold" placeholder="Nom du livre (ex: Genesis)" />
        <input type="number" min={1} value={chapitre} onChange={(e) => setChapitre(parseInt(e.target.value) || 1)} className="w-24 px-3 py-2 rounded-md border border-stone/30 bg-ivory text-ink text-sm focus:outline-none focus:border-gold" />
      </div>

      <div className="p-4 bg-imperial/5 border border-gold/20 rounded-md">
        <p className="text-xs text-stone leading-relaxed">
          <strong className="text-ink">Peshitta</strong> — Bible araméenne (syriaque). Langue parlée par Yeshoua et ses disciples.
          ...
          <div className="space-y-2">
            {data.versets.map((v) => (
              <div key={v.numero} className="flex gap-3">
                <span className="text-xs text-gold-dark font-semibold w-8 text-right pt-0.5">{v.numero}</span>
                <p className="text-sm text-ink/85 font-serif leading-relaxed" dir="rtl">{v.texte}</p>
              </div>
            ))}
          </div>
        </div>
      ) : (
        <p className="text-stone italic text-center py-8">Chapitre non trouvé.</p>
      )}
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div className="flex gap-3">
        <input
          ...
          className="flex-1 px-4 py-2 rounded-md border border-stone/30 bg-ivory text-ink text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
        />
        <button onClick={handleSearch} className="px-5 py-2 rounded-md bg-gold text-ink text-sm font-semibold hover:bg-gold-light">
          Chercher
        </button>
      </div>
      ...
      <div className="p-4 bg-imperial/5 border border-gold/20 rounded-md">
        <p className="text-xs text-stone leading-relaxed">
          <strong className="text-ink">Concordance Strong</strong> — Trouve tous les versets de la Bible hébraïque
          ...
          {data.strong && (
            <div className="card-gold-top p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="px-2 py-0.5 rounded text-xs font-bold bg-gold/15 text-gold-dark">{data.strong.numero as string}</span>
                {Boolean(data.strong.lemma) && <span className="font-serif text-lg text-ink" dir="rtl">{data.strong.lemma as string}</span>}
              </div>
              {Boolean(data.strong.strongs_def) && <p className="text-sm text-ink/70">{data.strong.strongs_def as string}</p>}
            </div>
          )}

          <p className="text-xs text-stone">{data.versets.length} verset(s) trouvé(s)</p>
                ...
                <div className="flex flex-wrap gap-1" dir="rtl">
                  {v.mots.map((m, j) => (
                    <span
                      ...
                      className={cn(
                        "font-serif text-sm",
                        m.lemme.includes(numero.replace("H", "")) ? "text-gold-dark font-bold" : "text-ink/60"
                      )}
                    >
                      {m.mot}
                    </span>
