...
const SERVANTS_MOCK: ServantData[] = [
...
export default function Home() {
  ...
  useEffect(() => {
    // Fetch data from API — if it fails, keep mock data
    fetch("/api/home")
  ...
  const pam = data.servants.find((s) => s.code === "pam");
  const kongo = data.servants.find((s) => s.code === "kongo");
  const latestTeaching = data.teachings[0];
  ...
  return (
    <div>
      <CinematicHero
        ...
        isLive={!!data.videos.find((v) => v.isLive)}
      />
      ...
      <section className="bg-ivory py-24 md:py-32 relative overflow-hidden">
        ...
        <div className="relative container mx-auto max-w-7xl px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <p className="text-xs uppercase tracking-[0.3em] text-stone font-semibold mb-3">Ce que cette plateforme rassemble</p>
            <h2 className="font-serif text-3xl md:text-4xl font-semibold text-ink">Une communauté en <TextShimmer>activité</TextShimmer></h2>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
            {stats.map((stat, i) => (
              <div key={i} className="text-center pt-4">
                <div className="font-serif text-4xl md:text-5xl lg:text-6xl font-semibold text-imperial mb-2 tracking-tight">{stat.value}</div>
                <div className="text-xs md:text-sm text-stone leading-snug max-w-[120px] mx-auto">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-ivory py-24 md:py-32 relative">
        <div className="container mx-auto max-w-7xl px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-16">
            <p className="text-xs uppercase tracking-[0.3em] text-gold font-semibold mb-3">Deux ministères, une même vision</p>
            <h2 className="font-serif text-3xl md:text-5xl font-semibold text-ink leading-tight">Deux voix, une même <TextShimmer>vision</TextShimmer></h2>
            <p className="mt-5 text-base md:text-lg text-stone leading-relaxed max-w-2xl mx-auto">
              PAM et le Pasteur Kongo exercent chacun un ministère distinct, uni par le mariage et par une même conviction : préparer les cœurs, transmettre ce qui a été reçu, et rassembler ceux qui se reconnaissent dans cette parole.
            </p>
          </motion.div>
            ...
            {pam && (
              <TiltCard maxTilt={8} className="h-full">
                <SpotlightCard className="h-full p-8 bg-ivory">
                  <ServantCardContent portrait="AP" name={pam.shortName} fullName={pam.fullName} role={pam.role} bio={pam.bio} href="/biographie" ctaLabel="Lire la biographie de PAM" />
                </SpotlightCard>
              </TiltCard>
            )}
            {kongo && (
              <TiltCard maxTilt={8} className="h-full">
                <SpotlightCard className="h-full p-8 bg-ivory">
                  <ServantCardContent portrait="PK" name={kongo.shortName} fullName={kongo.fullName} role={kongo.role} bio={kongo.bio} href="/biographie" ctaLabel="Lire la biographie du Pasteur Kongo" />
                </SpotlightCard>
              </TiltCard>
            )}
          </div>
      ...
      <AuroraBackground variant="imperial" intensity="strong" className="py-32 md:py-40">
        ...
        <div className="relative">
          <QuoteBlock text="Et Hénoch marcha avec Dieu ; et il ne fut plus, car Dieu le prit." reference="Genèse 5:24" variant="dark" />
        </div>
      </AuroraBackground>
        ...
        <div className="relative container mx-auto max-w-4xl px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <MagneticButton href="/communaute" variant="primary"><Hash className="w-4 h-4" />Rejoindre un canal</MagneticButton>
              <MagneticButton href="/contribuer" variant="secondary"><BookOpen className="w-4 h-4" />Contribuer</MagneticButton>
            </div>
          </motion.div>
        </div>
      </AuroraBackground>
...
function ServantCardContent({ portrait, name, fullName, role, bio, href, ctaLabel }: { portrait: string; name: string; fullName: string; role: string; bio: string; href: string; ctaLabel: string }) {
  return (
    <div className="flex flex-col h-full">
      ...
      <Link href={href} className="inline-flex items-center gap-1.5 text-sm font-semibold text-imperial hover:text-gold transition-colors group/cta mt-auto">
        {ctaLabel}
