...
const STATUS_CONFIG = {
  ...
  SCHEDULED: { label: "Programmé", color: "bg-[#C9A227]/15 text-[#DDBE55] border border-[#C9A227]/30", pulse: false },
  ENDED: { label: "Terminé", color: "bg-[#1A0826]/60 text-[#BDB4C9]", pulse: false },
  CANCELLED: { label: "Annulé", color: "bg-red-400/15 text-red-400", pulse: false },
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-[#1A0826]/70 rounded-2xl shadow-lg border border-red-400/25/50 p-4">
          <div className="text-2xl font-bold text-red-400">{stats.live}</div>
          <div className="text-[10px] uppercase tracking-wider text-red-400 font-semibold mt-0.5">En direct</div>
        </div>
        <div className="bg-[#1A0826]/70 rounded-2xl border border-[#C9A227]/40 shadow-lg p-4">
          <div className="text-2xl font-bold text-[#DDBE55]">{stats.upcoming}</div>
          <div className="text-[10px] uppercase tracking-wider text-[#DDBE55] font-semibold mt-0.5">À venir</div>
        </div>
        <div className="bg-[#1A0826]/70 rounded-2xl shadow-lg border border-[#C9A227]/15 p-4">
          <div className="text-2xl font-bold text-[#BDB4C9]">{stats.ended}</div>
          <div className="text-[10px] uppercase tracking-wider text-[#BDB4C9] font-semibold mt-0.5">Terminés</div>
        </div>
      </div>
      ...
      {lives.length === 0 ? (
        <div className="bg-[#1A0826]/70 rounded-2xl shadow-xl border border-dashed border-[#C9A227]/30 p-12 text-center">
          <Radio className="w-10 h-10 text-[#FAF6EF]/20 mx-auto mb-3" />
          <p className="text-sm text-[#BDB4C9] italic">Aucun live programmé pour l&apos;instant.</p>
        </div>
      ) : (
              ...
              >
                <div className="flex items-start gap-4 flex-wrap">
                  ...
                  >
                    ...
                    {status.pulse && (
                      <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-red-400/100 animate-pulse" />
                    )}
                  </div>
                  ...
                  <div className="min-w-0 flex-1 basis-[min(100%,16rem)]">
                    <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
                      <h3 className="font-bold text-sm text-[#FAF6EF] min-w-0 break-words">{l.title}</h3>
                    ...
                    <div className="flex items-center gap-3 text-[11px] text-[#BDB4C9] mt-2 flex-wrap">
                      ...
                      {estRubrique(l.category) && (
                        <>
                          ...
                          <span
                            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#C9A227]/15 text-[#DDBE55] font-bold"
                      ...
                      {isUpcoming && (
                        <>
                          <span className="text-[#FAF6EF]/25">·</span>
                          <span className="text-[#A3C9B0] font-semibold">
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {(l.status === "SCHEDULED" || l.status === "LIVE") && (
                      <Link
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-[#3D1A54] text-[#FAF6EF] text-xs font-bold hover:bg-[#3D1A54] transition-colors"
                    ...
                    {l.status === "LIVE" && (
                      <Link
                        href={`/live/${l.id}`}
                        target="_blank"
                        className="inline-flex items-center justify-center min-w-[44px] min-h-[44px] rounded-lg hover:bg-red-400/10 text-red-400 transition-colors"
