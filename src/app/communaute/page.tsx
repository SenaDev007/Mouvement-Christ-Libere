...
export default async function CommunautePage() {
  ...
  const roles = [
    ...
    { role: "Modérateur", holder: "Bénévoles validés", color: "bg-[#8A857C]" },
    { role: "Membre", holder: "Croyant inscrit", color: "bg-[#8A857C]" },
  ];
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* Rôles */}
      <section className="py-20 bg-[#F0E9DE] border-b border-[#8A857C]/10">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-xs uppercase tracking-[0.18em] text-[#8A857C] font-semibold mb-6 text-center">
            Rôles dans la communauté
          </p>
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {roles.map((r) => (
              <div key={r.role} className="flex items-center gap-3 p-4 rounded-2xl border border-[#8A857C]/20 bg-white">
                <span className={`w-3 h-3 rounded-full ${r.color}`} />
                <div>
                  <p className="text-sm font-semibold text-[#000000]">{r.role}</p>
                  <p className="text-xs text-[#8A857C]">{r.holder}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Canaux */}
      <section className="py-24 bg-[#F0E9DE] relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4">
      ...
      <section className="py-24 bg-[#000000] relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          <h2 className="font-serif text-3xl sm:text-4xl md:text-5xl font-extrabold text-[#F0E9DE] leading-tight mb-6">
            Le chat en temps réel
          </h2>
          <p className="text-base md:text-lg text-[#F0E9DE]/60 leading-relaxed max-w-2xl mx-auto mb-10">
      ...
      {/* Citation */}
      <section className="py-20 bg-[#000000] border-t border-[#C9A227]/20">
